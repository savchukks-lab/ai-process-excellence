...
from investment_financing import (
    FINANCING_PERIODS,
    calculate_all_financing_scenarios,
...
def inject_css() -> None:
    st.markdown(
        ...
        .launch-funnel th:first-child,
        .launch-funnel th:nth-child(2),
        .launch-funnel th:nth-child(3) { text-align: left; }
        .launch-funnel td {
        ...
        .launch-funnel td:first-child,
        .launch-funnel td:nth-child(2),
        .launch-funnel td:nth-child(3) { text-align: left; }
        .launch-funnel tbody tr:nth-child(even) { background: #f8fafc; }
        .launch-funnel tbody tr.launch-key-output { background: #f2f7f5; }
        .launch-funnel tbody tr.launch-key-output td:first-child { font-weight: 700; color: #174f3f; }
        .launch-funnel-type {
            border: 1px solid #d7dee8;
            border-radius: 999px;
            color: #526173;
        ...
        .launch-funnel-type.reference { background: #f1f5f9; }
        .launch-funnel-type.input { background: #fff8e8; border-color: #ead7a7; color: #775c18; }
        .launch-funnel-type.calculated { background: #edf7f3; border-color: #bddccf; color: #22634e; }
        div[data-testid="stColumn"]:has(.launch-confirm-marker) button[kind="primary"],
        div[data-testid="stColumn"]:has(.launch-confirm-marker) button[kind="primary"]:hover,
        div[data-testid="stColumn"]:has(.launch-confirm-marker) button[kind="primary"]:focus,
        div[data-testid="stColumn"]:has(.launch-confirm-marker) button[kind="primary"]:active,
        div[data-testid="stColumn"]:has(.launch-confirm-marker) button[kind="primary"] * {
    ...
    rows = [
        {
            ...
            "Critical Open Issues": "Coverage timing and eligible population assumptions need cross-functional alignment.",
        },
        {
...
def launch_workstreams(case_id: str, assumptions: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    for workstream in LAUNCH_WORKSTREAMS:
        ...
        if not owned.empty:
            statuses = owned["Validation Status"].astype(str).map(launch_alignment_status)
            input_status = "Inputs captured" if statuses.isin(["Draft", "Shared for Alignment", "Aligned"]).all() else "Inputs required"
        rows.append(
            {
                "Workstream": workstream,
                "Owner": workstream,
                "Input Status": input_status,
                "Alignment Status": "Not calculated",
...
def launch_responsibility_matrix(assumptions: pd.DataFrame | None = None) -> pd.DataFrame:
    if assumptions is not None and not assumptions.empty:
        return assumptions[["Assumption Name", "Owner", "Validators"]].rename(
            columns={"Assumption Name": "Assumption / Input", "Validators": "Alignment Partners"}
        )
    rows = [
...
def central_reference(reference_type: str, reference_key: str, fallback_key: str | None = None) -> dict[str, object]:
    references = CENTRAL_REFERENCE_LIBRARY.get(reference_type, {})
    selected_key = reference_key if reference_key in references else fallback_key
    if selected_key not in references:
        return {}
    reference = deepcopy(references[selected_key])
    reference.update({"Reference Type": reference_type, "Reference Key": selected_key, "Source": "Central Reference Data"})
    return reference


def central_population_reference(case: pd.Series) -> dict[str, object]:
    market = str(case.get("Market / Region", "Region A"))
    reference = central_reference("Population", market, "Region A")
...
LAUNCH_DEFINITIONS = {
    ...
    "Market Share": "Share of commercially accessible patients expected to receive our product.",
    ...
    "List Price": "Public / official / registered price used as the visible reference price for the funding channel.",
    "Net Price": "Realized commercial price recognized in revenue after discounts, rebates and other price adjustments.",
...
def launch_definition(name: object) -> str:
    return LAUNCH_DEFINITIONS.get(str(name), "")


def launch_definition_label(name: object) -> str:
    text = str(name)
    definition = launch_definition(text)
    if not definition:
        return escape(text)
    return f"{escape(text)} <span class='launch-info' title='{escape(definition, quote=True)}'>ⓘ</span>"


def launch_alignment_status(value: object) -> str:
    return {
        "Submitted for Validation": "Shared for Alignment",
        "Submitted for Alignment": "Shared for Alignment",
    }.get(str(value), str(value))


def launch_alignment_partners(assumption: pd.Series | dict[str, object]) -> list[str]:
    return split_validators(assumption.get("Validators", ""))


def launch_owner_caption(assumption: pd.Series | dict[str, object]) -> str:
    owner = str(assumption.get("Owner", ""))
    partners = launch_alignment_partners(assumption)
    return f"Owner: {owner}" + (f" · Alignment with: {', '.join(partners)}" if partners else "")
...
def build_launch_case_assumptions(case: pd.Series, product: dict[str, object]) -> list[dict[str, object]]:
    ...
    def add(
        ...
        row: dict[str, object] = {
            ...
            "Assumption Type": "CALCULATED OUTPUT" if calculated else assumption_type,
            "Enabled": enabled,
            "Validation History": [],
            "Options": options or [],
            "Last Updated": date.today().isoformat(),
            "Alignment Schema Version": 3,
    ...
    add("Medical", "Market Opportunity", "Treatment Rate / Treatment Eligibility", "", "Yearly Percentage", "%", "Marketing, Regulatory", launch_year_values(treatment_rate), assumption_type="EVIDENCE-BASED", source="Treatment pathway evidence")
    ...
    add("Marketing", "Adoption", "Market Share", "", "Yearly Percentage", "%", "Sales", inputs["market_share"], assumption_type="MANAGEMENT / FUNCTIONAL", rationale="Share of commercially accessible patients expected to receive our product.")
    ...
    add(
        ...
        [
            {
                "Competitor / Event Name": "Expected competitor launch",
                "Event Type": "New Competitor Entry",
                "Expected Year": "Y2",
                "Market Share Impact Y1": 0.0,
                "Market Share Impact Y2": -2.0,
                "Market Share Impact Y3": -3.0,
                "Market Share Impact Y4": -4.0,
                "Market Share Impact Y5": -5.0,
                "Comment": "Monitor timing and account-level response.",
            }
        ],
        "Structured Events",
        validators="Sales",
    ...
    add("Sales", "Sales Coverage & Execution Plan", "Sales FTE", "", "Yearly Integer", "FTE", "Marketing", yearly=sales["Sales Force HC / FTE"], assumption_type="MANAGEMENT / FUNCTIONAL", rationale=sales_rationale)
    add("Sales", "Sales Coverage & Execution Plan", "Regions Covered", "", "Yearly Integer", "Regions", "Marketing", yearly=launch_year_values(0, "By Year", [2, 3, 4, 5, 5]), assumption_type="MANAGEMENT / FUNCTIONAL", rationale=sales_rationale, enabled=True)
    add("Sales", "Sales Coverage & Execution Plan", "Population-weighted Geographic Coverage %", "", "Yearly Percentage", "%", "Marketing", yearly=launch_year_values(safe_float(sales["Coverage %"])), assumption_type="MANAGEMENT / FUNCTIONAL", rationale=sales_rationale, enabled=True)
    add("Sales", "Sales Coverage & Execution Plan", "Target Account / Center Coverage %", "", "Yearly Percentage", "%", "Marketing", yearly=launch_year_values(0, "By Year", [0.35, 0.50, 0.62, 0.72, 0.78]), assumption_type="MANAGEMENT / FUNCTIONAL", rationale=sales_rationale, enabled=True)
    ...
    add("Medical", "Target Patient & Treatment Positioning", "Target Segment / Severity", "Clinically appropriate target segment", "Text", validators="Marketing, Regulatory")
    ...
    add("Medical", "Target Patient & Treatment Positioning", "Biomarker / Prior Treatment / Clinical Restriction", "No additional restriction assumed", "Text", validators="Marketing, Regulatory")
    add("Medical", "Target Patient & Treatment Positioning", "Short Treatment Pathway Description", "Diagnosed eligible patients progress to the modeled treatment population.", "Text", validators="Marketing, Regulatory")
    add("Medical", "Target Patient & Treatment Positioning", "Other Population-defining Criteria", "", "Text", validators="Marketing, Regulatory")
    ...
    add("Medical", "Treatment & Utilization", "Treatment Duration", safe_float(utilization.get("Treatment Duration", utilization.get("Units per Patient", 0))), "Number", str(utilization.get("Treatment Duration Unit", "months")), rationale="Expected treatment duration for the modeled target patient.")
    records[-1]["Duration Unit"] = str(utilization.get("Treatment Duration Unit", "months"))
    add("Medical", "Treatment & Utilization", "Compliance", utilization.get("Compliance", 1), "Percentage", "%", rationale="Expected share of planned administrations taken as intended.")
    add("Medical", "Treatment & Utilization", "Persistence", utilization.get("Persistence", 1), "Percentage", "%", rationale="Optional adjustment for patients remaining on treatment.", enabled=bool(utilization.get("Persistence Enabled", False)))
    ...
    add("Medical", "Key Clinical Value", "Unmet Need Level", "High", "Choice", options=["Low", "Medium", "High"], rationale="Current treatment options leave a meaningful gap for the target population.")
    add("Medical", "Key Clinical Value", "Current Treatment Gap / Rationale", "Current options do not fully address clinically important outcomes for the target population.", "Text")
    add("Medical", "Key Clinical Value", "Main Clinical Comparators", "Current standard of care and the principal alternative used in the target treatment setting.", "Text")
    add("Medical", "Key Clinical Value", "Evidence Maturity", "Emerging", "Choice", options=["Early", "Emerging", "Established"])
    add("Medical", "Key Clinical Value", "Absolute Efficacy", "Available evidence indicates clinically meaningful efficacy in the target population.", "Text")
    add("Medical", "Key Clinical Value", "Comparative Efficacy", "Evidence suggests a differentiated efficacy profile versus relevant current alternatives.", "Text")
    add("Medical", "Key Clinical Value", "Efficacy Summary", "The efficacy proposition is relevant to the principal remaining treatment gap.", "Text")
    add("Medical", "Key Clinical Value", "Absolute Safety / Tolerability", "The known safety profile is manageable within the expected treatment setting.", "Text")
    add("Medical", "Key Clinical Value", "Comparative Safety / Tolerability", "Tolerability is expected to be competitive with the main clinical alternatives.", "Text")
    add("Medical", "Key Clinical Value", "Safety Summary", "No new safety limitation is expected to prevent use in the defined target population.", "Text")
    add("Medical", "Key Clinical Value", "Overall Clinical Value", "The product may provide meaningful clinical value for eligible patients while longer-term and comparative evidence continue to mature.", "Text")
    add("Medical", "Key Clinical Value", "Key Clinical Evidence Gap / Risk", "Long-term evidence maturity should be monitored through launch readiness.", "Text", validators="Regulatory", rationale="Primary evidence uncertainty that may affect positioning or readiness.")
    ...
    add("Market Access", "Access Strategy & Eligibility", "Access Archetype", case.get("Access Archetype", "Reimbursement Dependent"), "Choice", validators="Marketing", options=["Reimbursement Dependent", "Mixed Access", "Predominantly OOP / Broad Access"])
    ...
    add("Regulatory", "Regulatory Path & Timing", "Expected Regulatory Approval Date", regulatory.get("Expected Regulatory Approval Date", ""), "Date", validators="Market Access, Supply / Operations")
    add("Regulatory", "Regulatory Path & Timing", "Approval Timing Rationale / Key Drivers", regulatory.get("Approval Timing Rationale / Key Drivers", ""), "Text")
    add(
        "Regulatory",
    ...
    add("Regulatory", "Material Regulatory Risk", "Risk Level", regulatory.get("Risk Level", "Medium"), "Choice", options=["Low", "Medium", "High"])
    add("Regulatory", "Material Regulatory Risk", "Risk / Issue", regulatory.get("Risk / Issue", ""), "Text")
    add("Regulatory", "Material Regulatory Risk", "Potential Impact", regulatory.get("Potential Impact", ""), "Text")
    add("Regulatory", "Material Regulatory Risk", "Response / Management Consideration", regulatory.get("Response / Management Consideration", ""), "Text")
    ...
    add("Supply / Operations", "Manufacturing / Supply Constraints", "Manufacturing / Supply Constraint Status", supply.get("Constraint Status", "No known constraint"), "Choice", validators="Sales", options=["No known constraint", "At Risk", "Constraint confirmed"])
    ...
    add("Supply / Operations", "Supply / Warehouse Capacity", "Supply / Warehouse Capacity Status", supply.get("Warehouse Capacity Status", "Yes"), "Choice", options=["Yes", "At Risk", "No"])
...
def normalize_competitive_events(value: object) -> list[dict[str, object]]:
    normalized = []
    legacy_impact = {"High": -5.0, "Medium": -3.0, "Low": -1.0}
    event_type_aliases = {
        "LOE / Generic Entry": "Generic / Biosimilar Entry / LOE",
        "Generic Entry": "Generic / Biosimilar Entry / LOE",
        "Price Change": "Price / Access Change",
    }
    for raw_event in value if isinstance(value, list) else []:
        if not isinstance(raw_event, dict):
            continue
        event = {
            "Competitor / Event Name": str(raw_event.get("Competitor / Event Name", "")),
            "Event Type": event_type_aliases.get(str(raw_event.get("Event Type", "Other")), str(raw_event.get("Event Type", "Other"))),
            "Expected Year": str(raw_event.get("Expected Year", "Y1")),
            "Comment": str(raw_event.get("Comment", "")),
        }
        expected_year = event["Expected Year"] if event["Expected Year"] in LAUNCH_YEARS else "Y1"
        fallback_impact = legacy_impact.get(str(raw_event.get("Expected Impact", "")), 0.0)
        for year in LAUNCH_YEARS:
            field = f"Market Share Impact {year}"
            event[field] = safe_float(raw_event.get(field, fallback_impact if LAUNCH_YEARS.index(year) >= LAUNCH_YEARS.index(expected_year) else 0.0))
...
def get_launch_assumption_records(case: pd.Series, product: dict[str, object]) -> list[dict[str, object]]:
    ...
    defaults = build_launch_case_assumptions(case, product)
    if key not in st.session_state:
    ...
    for default in defaults:
        ...
        if str(default.get("Assumption Type")) != "MASTER DATA" and "Owner" in existing:
        ...
        if name == "Manufacturing / Supply Constraint Status":
            merged["Value"] = {
                ...
                "Yes — material constraint": "Constraint confirmed",
        ...
        if str(default.get("Value Type", "")).startswith("Yearly"):
            ...
            for year in LAUNCH_YEARS:
                if has_years:
                    merged[year] = safe_float(existing.get(year, default.get(year, 0)))
                elif legacy_value:
                    merged[year] = legacy_value
        normalized.append(merged)
    if normalized != stored:
        st.session_state[key] = [deepcopy(row) for row in normalized]
    return [deepcopy(row) for row in normalized]


def set_launch_assumption_records(case_id: str, records: list[dict[str, object]]) -> None:
    st.session_state[launch_assumption_state_key(case_id)] = [deepcopy(row) for row in records]
...
def apply_launch_assumptions_to_inputs(inputs: dict[str, object], records: list[dict[str, object]]) -> dict[str, object]:
    ...
    updated["sales_resources"]["Target Account / Center Coverage %"] = years("Target Account / Center Coverage %", launch_year_values(0))
...
def calculate_patient_flow(flow_rows: list[dict[str, object]], scenario_multiplier: float) -> pd.DataFrame:
    ...
    calculated_labels = {
        ...
        "Treatment Rate / Treatment Eligibility": "Treated Patients",
    }
    for row in flow_rows:
        ...
        if input_type in {"absolute patients", "absolute population"}:
        ...
        if input_type not in {"absolute patients", "absolute population"}:
...
def calculate_launch_funnel(
    ...
    def yearly(name: str, year: str, fallback: float = 0.0) -> float:
        row = by_name.get(name, {})
        value = row.get(year)
    ...
    for year in LAUNCH_YEARS:
        ...
        overall_share = patients_on_product / treated_patients if treated_patients else 0.0
        rows.append(
            {
                "Year": year,
                "Population": round(population),
                "Prevalence": prevalence,
                "Disease Population": round(disease_population),
                ...
                "Overall Market Share": overall_share,
            }
        )
    return pd.DataFrame(rows)
...
def launch_scenario_adjustments(inputs: dict[str, object], scenario: str) -> dict[str, float]:
    defaults = {
        "Downside": {"Population": 0.85, "Treatment Eligibility": 1.0, "Access Rate": 0.90, "Market Share": 0.85, "Market Share PP": 0.0, "Net Price": 0.95, "COGS": 1.05, "Timing Shift Days": 180},
        "Base": {"Population": 1.0, "Treatment Eligibility": 1.0, "Access Rate": 1.0, "Market Share": 1.0, "Market Share PP": 0.0, "Net Price": 1.0, "COGS": 1.0, "Timing Shift Days": 0},
        "Upside": {"Population": 1.08, "Treatment Eligibility": 1.0, "Access Rate": 1.05, "Market Share": 1.12, "Market Share PP": 0.0, "Net Price": 1.03, "COGS": 0.98, "Timing Shift Days": -60},
    }
...
def get_launch_projects(case_id: str, default_projects: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in st.session_state[key]:
        ...
        prepared["Alignment Partner(s)"] = prepared.pop("Validator(s)", prepared.get("Alignment Partner(s)", "Finance"))
        prepared.setdefault("Source Type", "Internal Data")
    ...
    return [dict(row) for row in prepared_projects]
...
def calculate_launch_model_no_scenarios(
    data: dict[str, pd.DataFrame],
    case: pd.Series,
    scenario: str,
    adjustment_overrides: dict[str, object] | None = None,
...
def render_launch_model_sections(case_id: str, case: pd.Series, data: dict[str, pd.DataFrame], model: dict[str, object]) -> None:
    ...
    st.dataframe(model.get("funnel", pd.DataFrame()), use_container_width=True, hide_index=True)
    st.caption("Patient-funnel inputs are maintained by their functional owners in Workstreams. Calculated patient outputs are system-generated.")
    ...
    resource_view = pd.DataFrame(
        [
            {"Resource Assumption": "Sales FTE", **{year: f"{safe_float(value):,.0f}" for year, value in sales_resources["Sales Force HC / FTE"].items()}},
            {"Resource Assumption": "Regions Covered", **{year: f"{safe_float(value):,.0f}" for year, value in sales_resources["Regions Covered"].items()}},
            {"Resource Assumption": "Population-weighted Geographic Coverage %", **{year: pct(value) for year, value in sales_resources["Population-weighted Geographic Coverage %"].items()}},
            {"Resource Assumption": "Target Account / Center Coverage %", **{year: pct(value) for year, value in sales_resources["Target Account / Center Coverage %"].items()}},
            {"Resource Assumption": "Target Accounts / Centers", **{year: f"{safe_float(value):,.0f}" for year, value in sales_resources["Target Accounts / Centers"].items()}},
        ]
    )
    ...
    with resource_tabs[2]:
        ...
        if bool(supply.get("Capacity Constraint Enabled", False)) and supply.get("Constraint Status") in {"At Risk", "Constraint confirmed", "Yes — material constraint"}:
...
def launch_assumption_display_value(assumption: pd.Series | dict[str, object]) -> str:
    ...
    if value_type.startswith("Yearly"):
        ...
        for year in LAUNCH_YEARS:
            ...
            number = f"{value:,.0f}" if value_type == "Yearly Integer" else f"{value:,.1f}"
            values.append(f"{year} {pct(value) if is_percentage else number}")
        return " · ".join(values)
...
def update_launch_assumption_record(case_id: str, assumption_id: str, updates: dict[str, object]) -> None:
    key = launch_assumption_state_key(case_id)
    records = [deepcopy(row) for row in st.session_state.get(key, [])]
    changed = False
    for index, record in enumerate(records):
        if str(record.get("Assumption ID")) != assumption_id:
            continue
        actual_updates = {field: value for field, value in updates.items() if record.get(field) != value}
        if actual_updates:
...
def update_launch_validation(case_id: str, assumption_id: str, action: str, comment: str = "") -> None:
    key = launch_assumption_state_key(case_id)
    records = [deepcopy(row) for row in st.session_state.get(key, [])]
    status_by_action = {
        "Submitted for Validation": "Shared for Alignment",
        "Shared for Alignment": "Shared for Alignment",
        "Confirmed": "Aligned",
        "Confirmed Alignment": "Aligned",
        "Requested Change": "Alignment Required",
    }
    for index, record in enumerate(records):
        if str(record.get("Assumption ID")) != assumption_id:
            continue
        ...
        for event in history:
            ...
            if str(assumption.get("Value Type", "")) == "Structured Evidence" and (old_events or new_events):
                ...
                for name in sorted(new_by_name.keys() - old_by_name.keys()):
                    ...
                    impacts = " · ".join(f"{year} {safe_float(item.get(f'Market Share Impact {year}')):+g}pp" for year in LAUNCH_YEARS)
                    st.write(f"Added: {name} · {item.get('Event Type', '')} · {item.get('Expected Year', '')}")
                    st.caption(impacts)
...
def render_competitive_events_input(case_id: str, assumption: pd.Series, editable: bool) -> None:
    ...
    events = normalize_competitive_events(assumption.get("Value", []))
    if events:
        display_events = pd.DataFrame(events)
        for year in LAUNCH_YEARS:
            field = f"Market Share Impact {year}"
            display_events[field] = display_events[field].map(lambda value: f"{safe_float(value):+g}pp")
        render_finance_table(display_events, {f"Market Share Impact {year}" for year in LAUNCH_YEARS})
        st.caption("Market Share Impact is entered in percentage points. For example, -2 pp reduces modeled product market share by two percentage points.")
    else:
        st.caption("No competitive events have been recorded.")
    with st.expander("+ Add Competitive Event"):
        columns = st.columns(2)
        name = columns[0].text_input("Competitor / Event Name", key=f"launch_comp_name_{case_id}", disabled=not editable)
        event_type = columns[1].selectbox(
            "Event Type",
            ["New Competitor Entry", "Generic / Biosimilar Entry / LOE", "New Indication", "Major Evidence Readout", "Price / Access Change", "Other"],
            key=f"launch_comp_type_{case_id}",
            disabled=not editable,
        )
        expected_year = st.selectbox("Expected Year", LAUNCH_YEARS, key=f"launch_comp_year_{case_id}", disabled=not editable)
        st.caption("Expected Market Share Impact (percentage points)")
        impact_columns = st.columns(5)
        impacts = {
            year: impact_columns[index].number_input(
                year,
                min_value=-100.0,
                max_value=100.0,
                value=0.0,
                step=1.0,
                format="%.1f",
                key=f"launch_comp_impact_{case_id}_{year}",
                disabled=not editable,
            )
            for index, year in enumerate(LAUNCH_YEARS)
        }
        comment = st.text_area("Comment", key=f"launch_comp_comment_{case_id}", height=70, disabled=not editable)
        if st.button("Add Competitive Event", key=f"launch_comp_add_{case_id}", disabled=not editable or not name.strip()):
            updated_events = events + [{
                "Competitor / Event Name": name.strip(),
                "Event Type": event_type,
                "Expected Year": expected_year,
                **{f"Market Share Impact {year}": safe_float(impacts[year]) for year in LAUNCH_YEARS},
                "Comment": comment.strip(),
            }]
...
def render_launch_assumption_input(case_id: str, assumption: pd.Series) -> None:
    assumption_id = str(assumption.get("Assumption ID", ""))
    assumption_name = str(assumption.get("Assumption Name", ""))
    ...
    st.markdown(f"<strong>{launch_definition_label(assumption_name)}</strong>", unsafe_allow_html=True)
    st.caption(f"{launch_owner_caption(assumption)} · Alignment status: {launch_alignment_status(assumption.get('Validation Status', 'Draft'))}")
    history = [item for item in assumption.get("Validation History", []) if isinstance(item, dict)]
    if str(assumption.get("Validation Status", "")) == "Alignment Required" and history:
        latest_comment = next((str(item.get("Comment", "")) for item in reversed(history) if item.get("Comment")), "")
        st.warning(f"Alignment required{f': {latest_comment}' if latest_comment else '.'}")
    ...
    if value_type.startswith("Yearly"):
        ...
        if mode == "Constant Across Forecast":
            ...
            number_kwargs = {"step": 1.0, "format": "%.0f"} if value_type == "Yearly Integer" else {}
            entered = st.number_input("Forecast Value", value=float(base), key=f"{key_base}_constant", disabled=not editable, **number_kwargs)
            for year in LAUNCH_YEARS:
            ...
            for index, year in enumerate(LAUNCH_YEARS):
                ...
                number_kwargs = {"step": 1.0, "format": "%.0f"} if value_type == "Yearly Integer" else {}
                entered = year_cols[index].number_input(year, value=float(current), key=f"{key_base}_{year}", disabled=not editable, **number_kwargs)
                updates[year] = safe_float(entered) / 100 if is_pct else safe_float(entered)
    ...
    assumption_type = str(assumption.get("Assumption Type", "MANAGEMENT / FUNCTIONAL"))
    if assumption_type == "MASTER DATA":
        st.caption(f"Master / external data · Source: {assumption.get('Source', 'Reference data')}")
    elif assumption_type == "EVIDENCE-BASED":
        with st.expander("Evidence and rationale"):
        ...
        with st.expander("Rationale"):
            rationale_label = "Coverage & Resourcing Rationale *" if assumption.get("Assumption Name") == "Sales FTE" else "Rationale / Comment *"
            updates["Rationale / Comment"] = st.text_area(rationale_label, value=str(assumption.get("Rationale / Comment", "")), key=f"{key_base}_rationale", disabled=not editable, height=90)
...
def launch_key_outputs(workstream: str, model: dict[str, object]) -> None:
    ...
    sales_resources = model.get("inputs", {}).get("sales_resources", {})
    geographic_coverage = safe_float(sales_resources.get("Population-weighted Geographic Coverage %", {}).get("Y5", 0))
    account_coverage = safe_float(sales_resources.get("Target Account / Center Coverage %", {}).get("Y5", 0))
    primary_coverage = geographic_coverage or account_coverage
    metrics = {
...
def render_launch_funnel(model: dict[str, object], assumptions: pd.DataFrame) -> None:
    funnel = model.get("funnel", pd.DataFrame())
    if not isinstance(funnel, pd.DataFrame) or funnel.empty:
        st.info("The launch funnel is not available for this case.")
        return
    ownership = {str(row.get("Assumption Name")): str(row.get("Owner")) for _, row in assumptions.iterrows()}
    funnel_order = [
        "Population",
        "Prevalence",
        "Disease Population",
    ...
    headers = ["Funnel Step", "Type", "Owner", *LAUNCH_YEARS]
    header_html = "".join(f"<th>{escape(header)}</th>" for header in headers)
    body_html = []
    for row in rows:
        metric = str(row["Funnel Step"])
        type_text = str(row["Type"])
        type_class = "reference" if type_text == "REFERENCE DATA" else "calculated" if type_text == "CALCULATED OUTPUT" else "input"
        row_class = "launch-key-output" if metric in key_outputs else ""
        cells = [
            f"<td>{launch_definition_label(metric)}</td>",
            f"<td><span class='launch-funnel-type {type_class}'>{escape(type_text)}</span></td>",
            f"<td>{escape(str(row['Owner']))}</td>",
            *[f"<td>{escape(str(row[year]))}</td>" for year in LAUNCH_YEARS],
        ]
        body_html.append(f"<tr class='{row_class}'>{''.join(cells)}</tr>")
    st.markdown(
        f"<div class='launch-funnel-wrap'><table class='launch-funnel'><thead><tr>{header_html}</tr></thead>"
...
def render_launch_validation_queue(case_id: str, assumptions: pd.DataFrame, workspace: str) -> None:
    allowed = {
        ...
        "Medical": {"Expected Label / Indication", "Access Eligibility / Restrictions"},
        "Market Access": {
        ...
        "Supply / Operations": {"Expected Regulatory Approval Date"},
    ...
    if workspace in allowed:
        queue = queue[queue["Assumption Name"].isin(allowed[workspace])]
    if queue.empty:
    ...
    for _, assumption in queue.iterrows():
        ...
        alignment_status = launch_alignment_status(assumption.get("Validation Status", "Draft"))
        title = (
            f"{assumption.get('Assumption Name', '')} · Owner: {assumption.get('Owner', '')} · "
            f"Status: {alignment_status} · {compact_summary}"
        )
        with st.expander(title, expanded=False):
            if str(assumption.get("Value Type", "")) == "Structured Events":
                events = [item for item in assumption.get("Value", []) if isinstance(item, dict)]
                if events:
                    display_events = pd.DataFrame(normalize_competitive_events(events))
                    for year in LAUNCH_YEARS:
                        field = f"Market Share Impact {year}"
                        display_events[field] = display_events[field].map(lambda value: f"{safe_float(value):+g}pp")
...
def launch_alignment_package_rows(
    assumptions: pd.DataFrame,
    workspace: str,
    names: set[str],
) -> pd.DataFrame:
    validators = assumptions["Validators"].astype(str).map(lambda value: workspace in split_validators(value))
    return assumptions[validators & assumptions["Assumption Name"].isin(names) & ~assumptions["Owner"].eq(workspace)].copy()


def render_business_alignment_package(
    case_id: str,
    workspace: str,
    package_name: str,
    ...
    if package.empty:
        return
    status = launch_package_status(package)
    key_name = re.sub(r"[^A-Za-z0-9]+", "_", f"{workspace}_{package_name}").strip("_").lower()
    with st.expander(f"{package_name} · Status: {status}", expanded=False):
        render_finance_table(summary, {year for year in LAUNCH_YEARS if year in summary.columns})
        ...
        pending_rows = [
            ...
            if launch_alignment_status(row.get("Validation Status", "Draft")) in {"Shared for Alignment", "Alignment Required"}
        ]
        can_confirm = can_review and any(not launch_partner_has_confirmed(row, workspace) for row in pending_rows)
        if actions[0].button("Confirm Alignment", type="primary", key=f"launch_package_confirm_{case_id}_{key_name}", disabled=not can_confirm):
            for row in pending_rows:
                if not launch_partner_has_confirmed(row, workspace):
                    update_launch_validation(case_id, str(row.get("Assumption ID", "")), "Confirmed Alignment", comment)
            st.success(f"{package_name} aligned.")
            st.rerun()
        if actions[1].button("Request Change", key=f"launch_package_change_{case_id}_{key_name}", disabled=not can_review):
            if not comment.strip():
                st.warning("A comment is required to request a change.")
            else:
                for row in pending_rows:
                    update_launch_validation(case_id, str(row.get("Assumption ID", "")), "Requested Change", comment)
                st.success(f"Change requested for {package_name}.")
                st.rerun()
        if actions[2].button("Add Comment", key=f"launch_package_add_comment_{case_id}_{key_name}", disabled=not can_review or not comment.strip()):
            primary = pending_rows[0] if pending_rows else package.iloc[0]
            update_launch_validation(case_id, str(primary.get("Assumption ID", "")), "Added Comment", comment)
            st.success("Comment added.")
            st.rerun()
        ...
        for _, assumption in package.iterrows():
            for event in assumption.get("Validation History", []):
                if not isinstance(event, dict):
                    continue
                signature = (str(event.get("Timestamp", "")), str(event.get("Role", "")), str(event.get("Action", "")))
                if signature in seen:
                    continue
                seen.add(signature)
                events.append((signature[0], f"{signature[1]} · {signature[2]}", str(event.get("Comment", ""))))
        if events:
            st.markdown("#### Alignment History")
            for timestamp_text, event_label, event_comment in sorted(events, reverse=True)[:10]:
                st.markdown(f"**{event_label}**")
                if event_comment:
                    st.write(f'“{event_comment}”')
                st.caption(timestamp_text)


def medical_target_package_summary(package: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for _, assumption in package.iterrows():
        name = str(assumption.get("Assumption Name", ""))
        rows.append({"Input": name, "Current value": short_business_text(launch_assumption_display_value(assumption), "Not set", 150)})
    return pd.DataFrame(rows)


def sales_coverage_package_summary(package: pd.DataFrame) -> pd.DataFrame:
    rows = []
    percentage_metrics = {"Population-weighted Geographic Coverage %", "Target Account / Center Coverage %"}
    for _, assumption in package.iterrows():
        name = str(assumption.get("Assumption Name", ""))
        if name != "Sales FTE" and not bool(assumption.get("Enabled", True)):
            continue
        rows.append({
            "Input": name,
            **{
                year: pct(assumption.get(year)) if name in percentage_metrics else f"{safe_float(assumption.get(year)):,.0f}"
                for year in LAUNCH_YEARS
            },
        })
    return pd.DataFrame(rows)
...
def market_access_package_summary(package: pd.DataFrame) -> pd.DataFrame:
    plan_row = package[package["Assumption Name"].eq("Market Access Channel Plan")]
    if plan_row.empty:
        return medical_target_package_summary(package)
    plan = plan_row.iloc[0].get("Value", {}) if isinstance(plan_row.iloc[0].get("Value"), dict) else {}
    rows = []
    for entry in market_access_plan_entries(plan):
        rows.append({
            "Access path": entry.get("Access Path", entry.get("Channel", "")),
            "Access begins": entry.get("Access Start Date", ""),
...
def render_sales_plan_alignment(case_id: str, assumptions: pd.DataFrame) -> None:
    sales_plan = assumptions[
        assumptions["Category"].eq("Sales Coverage & Execution Plan")
        & assumptions["Owner"].eq("Sales")
        & (assumptions["Assumption Name"].eq("Sales FTE") | assumptions["Enabled"].astype(bool))
    ]
    if sales_plan.empty:
        return
    statuses = sales_plan["Validation Status"].map(launch_alignment_status)
    if not statuses.eq("Shared for Alignment").any():
        return
    table_rows = []
    percentage_metrics = {"Population-weighted Geographic Coverage %", "Target Account / Center Coverage %"}
    ...
    with st.expander("Sales Coverage & Execution Plan · Owner: Sales · Status: Shared for Alignment", expanded=False):
        st.table(pd.DataFrame(table_rows).set_index("Metric"))
        rationale = str(sales_plan.iloc[0].get("Rationale / Comment", ""))
        if rationale:
            st.caption(f"Execution Strategy & Rationale: {rationale}")
        ...
        if actions[0].button("Confirm Alignment", type="primary", key=f"launch_sales_plan_confirm_{case_id}", disabled=not can_align):
            ...
            st.success("Sales Coverage & Execution Plan aligned.")
            st.rerun()
        if actions[1].button("Request Change", key=f"launch_sales_plan_change_{case_id}", disabled=not can_align):
            if not comment.strip():
                st.warning("A comment is required to request a change.")
            else:
                for _, assumption in sales_plan.iterrows():
                    update_launch_validation(case_id, str(assumption.get("Assumption ID", "")), "Requested Change", comment)
                st.success("Change requested from Sales.")
                st.rerun()
        if actions[2].button("Add Comment", key=f"launch_sales_plan_comment_{case_id}", disabled=not can_align or not comment.strip()):
            primary = sales_plan[sales_plan["Assumption Name"].eq("Sales FTE")].iloc[0]
            update_launch_validation(case_id, str(primary.get("Assumption ID", "")), "Added Comment", comment)
            st.success("Comment added.")
            st.rerun()
...
def render_marketing_workspace(case: pd.Series, data: dict[str, pd.DataFrame], assumptions: pd.DataFrame) -> None:
    ...
    medical_package = launch_alignment_package_rows(refreshed_assumptions, "Marketing", medical_names)
    render_business_alignment_package(
        case_id,
        "Marketing",
        "Medical Target Patient Package",
        medical_package,
        medical_target_package_summary(medical_package),
    )
    sales_names = {"Sales FTE", "Regions Covered", "Population-weighted Geographic Coverage %", "Target Account / Center Coverage %", "Target Accounts / Centers"}
    sales_package = launch_alignment_package_rows(refreshed_assumptions, "Marketing", sales_names)
    render_business_alignment_package(
        case_id,
        "Marketing",
        "Sales Coverage & Execution Package",
        sales_package,
        sales_coverage_package_summary(sales_package),
    )
    access_names = {"Access Archetype", "Access Strategy & Pathway", "Market Access Channel Plan"}
    access_package = launch_alignment_package_rows(refreshed_assumptions, "Marketing", access_names)
    render_business_alignment_package(
        case_id,
        "Marketing",
        "Market Access Plan Package",
        access_package,
        market_access_package_summary(access_package),
    )
    ...
    for _, assumption in coverage.iterrows():
        ...
        for event in assumption.get("Validation History", []):
            if isinstance(event, dict):
                ...
                new_values = event.get("New Value") if isinstance(event.get("New Value"), dict) else {}
                if name != "Sales FTE" and (action != "Updated assumption" or set(new_values) == {"Rationale / Comment"}):
                    continue
                events.append((str(event.get("Timestamp", "")), name, value_type, event))
    if not events:
        return
    with st.expander("Alignment History"):
        for _, name, value_type, event in sorted(events, reverse=True)[:20]:
            action = {
                "Submitted for Validation": "Shared for Alignment",
                "Confirmed": "Confirmed Alignment",
            }.get(str(event.get("Action", "")), str(event.get("Action", "")))
            heading = "SALES UPDATED COVERAGE PLAN" if action == "Updated assumption" else f"SALES {action.upper()}"
            st.markdown(f"**{heading}**")
            old_values = event.get("Old Value") if isinstance(event.get("Old Value"), dict) else {}
            new_values = event.get("New Value") if isinstance(event.get("New Value"), dict) else {}
            changes = []
            for year in LAUNCH_YEARS:
                if year not in new_values:
                    continue
                old_value = old_values.get(year)
                new_value = new_values.get(year)
                old_text = pct(old_value) if value_type == "Yearly Percentage" else f"{safe_float(old_value):,.0f}"
                new_text = pct(new_value) if value_type == "Yearly Percentage" else f"{safe_float(new_value):,.0f}"
                changes.append(f"{year} {old_text} → {new_text}")
            if changes:
                st.write(f"{name}: {' · '.join(changes)}")
            if "Rationale / Comment" in new_values:
                st.write("Sales strategy and resourcing rationale updated.")
            if event.get("Comment") and "Rationale / Comment" not in new_values:
                st.write(f'“{event.get("Comment")}”')
            parsed_timestamp = pd.to_datetime(event.get("Timestamp"), errors="coerce")
            timestamp = parsed_timestamp.strftime("%d %b %Y · %H:%M") if not pd.isna(parsed_timestamp) else str(event.get("Timestamp", ""))
            st.caption(timestamp)
...
def render_sales_workspace(case: pd.Series, data: dict[str, pd.DataFrame], assumptions: pd.DataFrame) -> None:
    ...
    edited = st.data_editor(
        ...
        hide_index=True,
        use_container_width=True,
        height=max(110, 36 * (len(editor_source) + 1)),
        disabled=True if not can_edit else ["Metric"],
        column_config=column_config,
    )
    if any(name in percentage_metrics for name in table_source.get("Metric", pd.Series(dtype=str)).astype(str)):
        st.caption("Coverage percentages use whole-number entry: 60 means 60%.")
    ...
    if not sales_fte.empty:
        ...
        rationale = st.text_area(
            ...
            disabled=not can_edit,
            height=110,
            help="Explain the field-force sizing, target regions/accounts, coverage priorities, reach model, engagement frequency and ramp-up logic.",
        )
        if can_edit:
            for _, assumption in coverage.iterrows():
...
def render_medical_workspace(case: pd.Series, data: dict[str, pd.DataFrame], assumptions: pd.DataFrame) -> None:
    ...
    st.caption("Owner: Medical · Evidence-based rates that quantify the diagnosed and treatment-eligible patient opportunity.")
    with st.container(border=True):
    ...
    if medical_fte is not None:
        ...
        with st.container(border=True):
            ...
            st.caption("Finance applies the fully loaded cost per Medical / MSL FTE.")
...
def render_market_access_workspace(case: pd.Series, data: dict[str, pd.DataFrame], assumptions: pd.DataFrame) -> None:
    ...
    funnel = model.get("funnel", pd.DataFrame())
    if isinstance(funnel, pd.DataFrame) and not funnel.empty:
        access_impact = funnel[["Year", "Treated Patients", "Total Market Access Rate", "Accessible Patients"]].copy()
        access_impact["Treated Patients"] = access_impact["Treated Patients"].map(lambda value: f"{safe_float(value):,.0f}")
        access_impact["Total Market Access Rate"] = access_impact["Total Market Access Rate"].map(finance_pct_display)
        access_impact["Accessible Patients"] = access_impact["Accessible Patients"].map(lambda value: f"{safe_float(value):,.0f}")
        render_finance_table(access_impact, {"Treated Patients", "Total Market Access Rate", "Accessible Patients"})
    ...
    if isinstance(channels, pd.DataFrame) and not channels.empty:
        ...
        channel_summary = channels.pivot_table(index="Access Path", columns="Year", values="Net Revenue", aggfunc="sum", fill_value=0).reindex(columns=LAUNCH_YEARS, fill_value=0)
        channel_summary.loc["Total"] = channel_summary.sum(axis=0)
        channel_summary = channel_summary.reset_index().rename(columns={"Access Path": "Channel"})
        for year in LAUNCH_YEARS:
            channel_summary[year] = channel_summary[year].map(money)
        render_finance_table(channel_summary, set(LAUNCH_YEARS))
        with st.expander("Revenue Detail / Traceability", expanded=False):
            channel_detail = channels.copy()
            channel_detail["Net Revenue"] = channel_detail["Net Revenue"].map(money)
            render_finance_table(channel_detail, {"Reach", "List Price", "Net Price", "Sellable Units", "Net Revenue"})
...
def render_regulatory_workspace(case: pd.Series, data: dict[str, pd.DataFrame], assumptions: pd.DataFrame) -> None:
    ...
    medical_package = launch_alignment_package_rows(refreshed, "Regulatory", medical_names)
    render_business_alignment_package(
        case_id,
        "Regulatory",
        "Medical Target Patient Package",
    ...
    evidence_names = {"Key Clinical Evidence Gap / Risk"}
    evidence_package = launch_alignment_package_rows(refreshed, "Regulatory", evidence_names)
    render_business_alignment_package(
        case_id,
        "Regulatory",
        "Key Clinical Evidence Gap / Risk",
        evidence_package,
        medical_target_package_summary(evidence_package),
    )
...
def render_supply_workspace(case: pd.Series, data: dict[str, pd.DataFrame], assumptions: pd.DataFrame) -> None:
    ...
    st.markdown("### Supply Feasibility")
    st.caption("Confirm whether planned demand can be supplied and identify only material manufacturing or capacity constraints.")
    with st.container(border=True):
        constraint_options = ["No known constraint", "At Risk", "Constraint confirmed"]
        ...
        current_constraint = "Constraint confirmed" if current_constraint == "Yes — material constraint" else current_constraint
        ...
        constrained = selected_constraint in {"At Risk", "Constraint confirmed"}
        ...
        if isinstance(forecast, pd.DataFrame) and not forecast.empty:
            demand_summary = pd.DataFrame([{
                "Demand": "Integrated model demand",
                **{
                    year: f"{safe_float(forecast.loc[forecast['Year'].eq(year), 'Demand Units'].iloc[0]):,.0f}"
                    for year in LAUNCH_YEARS
                },
            }])
            render_finance_table(demand_summary, set(LAUNCH_YEARS))
            st.caption("Demand Units are read-only and supplied by the integrated launch model.")
        if constrained and use_capacity_limit and isinstance(forecast, pd.DataFrame) and not forecast.empty:
            ...
            render_finance_table(comparison, {"Demand Units", "Maximum Available Units", "Sellable Units"})
            constrained_years = [
...
def render_launch_workspace(case: pd.Series, data: dict[str, pd.DataFrame], workspace: str) -> None:
    ...
    if workspace == "Marketing":
        render_marketing_workspace(case, data, assumptions)
        return
    if workspace == "Sales":
...
def update_launch_responsibility_matrix(case_id: str, edited: pd.DataFrame) -> None:
    ...
    for record in records:
        ...
        if config is not None and not bool(record.get("Calculated", False)):
            record = {**record, "Owner": str(config.get("Owner", record.get("Owner", ""))), "Validators": str(config.get("Alignment Partners", record.get("Validators", "")))}
        updated_records.append(record)
    if updated_records != records:
        set_launch_assumption_records(case_id, updated_records)


def launch_pnl_value(model: dict[str, object], metric: str, year: str = "Y5") -> float:
    pnl = model.get("pnl", pd.DataFrame())
    if not isinstance(pnl, pd.DataFrame) or pnl.empty:
        return 0.0
    row = pnl[pnl["Metric"].astype(str).eq(metric)]
...
def launch_integrated_journey(model: dict[str, object]) -> None:
    st.markdown("### Integrated Launch Journey")
    st.caption("Patient Opportunity → Access → Adoption → Utilization → Revenue → Profit")
    funnel = model.get("funnel", pd.DataFrame())
    forecast = model.get("forecast", pd.DataFrame())
    if not isinstance(funnel, pd.DataFrame) or funnel.empty or not isinstance(forecast, pd.DataFrame) or forecast.empty:
        st.info("Integrated launch outputs are not yet available.")
        return
    funnel_by_year = funnel.set_index("Year")
    forecast_by_year = forecast.set_index("Year")
    pnl = model.get("pnl", pd.DataFrame())
    pnl_by_metric = pnl.set_index("Metric") if isinstance(pnl, pd.DataFrame) and not pnl.empty else pd.DataFrame()
    rows = []
    for metric in [
        "Treated Patients",
        "Accessible Patients",
        ...
        for year in LAUNCH_YEARS:
            if metric in {"Treated Patients", "Accessible Patients", "Patients on Product"}:
                value = funnel_by_year.loc[year, metric] if year in funnel_by_year.index else 0
...
def launch_functional_status(
    ...
    for workspace in LAUNCH_WORKSTREAMS:
        ...
        aligned = package[package["Validators"].astype(str).str.strip().ne("")] if not package.empty else package
        package_status = launch_package_status(aligned)
        alignment_status = package_status if not aligned.empty else "Not required"
...
def launch_key_milestones(case: pd.Series, model: dict[str, object]) -> pd.DataFrame:
    inputs = model.get("inputs", {})
    regulatory = inputs.get("regulatory", {}) if isinstance(inputs, dict) else {}
    supply = inputs.get("supply", {}) if isinstance(inputs, dict) else {}
    access_metrics = model.get("access_metrics", {})
...
def render_launch_overview(case: pd.Series, data: dict[str, pd.DataFrame], model: dict[str, object], assumptions: pd.DataFrame) -> None:
    ...
    launch_management_snapshot(case, model)
    ...
    issues = launch_open_items(case, assumptions, model)
    if issues:
        for issue in issues:
            st.warning(issue)
    else:
        st.success("No material open issue currently identified.")
    render_launch_benchmark_snapshot(str(case.get("Launch Case ID", "")), model)


def launch_sensitivity_state_key(case_id: str) -> str:
...
def format_investment_driver_table(table: pd.DataFrame, years: list[str]) -> pd.DataFrame:
    ...
    for index, row in formatted.iterrows():
        ...
        for year in years:
            ...
            if any(term in metric for term in ("Capacity", "Volume", "Units", "Patients")) and "%" not in metric:
                display = f"{value:,.0f}"
            elif "%" in metric:
                display = pct(value)
            else:
...
def investment_driver_display_kind(metric: str) -> str:
    """Classify model inputs for display only; calculation values stay unchanged."""
    if "%" in metric:
        return "Percentage"
    if "per Unit" in metric or "Unit Cost" in metric:
        return "Unit value"
    if any(term in metric for term in ("Capacity", "Volume", "Units", "Patients", "Headcount")):
        return "Count"
...
def investment_driver_display_value(value: float, kind: str) -> float:
...
def render_investment_driver_editor(
    ...
    for kind, kind_metrics in grouped_metrics.items():
        rows = []
        ...
        for metric in kind_metrics:
            ...
            for year in years:
                ...
                original_display_values[(metric, year)] = display_value
                row[year] = investment_driver_display_value(display_value, kind)
            rows.append(row)
        ...
        for _, row in updated_editor.iterrows():
            ...
            for year in years:
                ...
                displayed_seed = investment_driver_display_value(original_display_values[(metric, year)], kind)
                value = original_display_values[(metric, year)] if edited_value == displayed_seed else edited_value
                updated[metric][year] = value / 100 if "%" in metric else value
    return updated


def render_investment_series_method(
    case_id: str,
    section_key: str,
    metric: str,
    source: dict[str, dict[str, float]],
    ...
    investment_seed_widget(y1_key, safe_float(updated.get(metric, {}).get("Y1")))
...
def render_investment_revenue_inputs(
    case_id: str,
    section_key: str,
...
def render_investment_pnl(title: str, purpose: str, pnl: pd.DataFrame, years: list[str]) -> None:
...
def handle_investment_archetype_change(case_id: str) -> None:
    archetype = str(st.session_state.get(f"investment_{case_id}_archetype", CASE_ARCHETYPES[0]))
    all_inputs = dict(st.session_state.get("investment_case_inputs", {}))
    old = deepcopy(all_inputs.get(case_id, {}))
    fresh = default_investment_inputs({"Case ID": case_id, "Investment Case Name": old.get("settings", {}).get("Case Name", "Investment Case"), "Case Archetype": archetype})
    for field in ["Case Name", "Currency", "Base Year", "Forecast Horizon", "Investment Start Date", "Operational Start Date", "Applicable Tax Rate", "Planning Inflation", "Financial Scope"]:
        if field in old.get("settings", {}): fresh["settings"][field] = old["settings"][field]
    if old.get("capital"): fresh["capital"].update(old["capital"])
    fresh["settings"]["Case Archetype"] = archetype
    ...
    for driver in VALUE_CREATION_DRIVERS:
        ...
        st.session_state[f"investment_{case_id}_driver_{driver_key}"] = driver in selected
...
def render_investment_record_editor(case_id: str, key: str, rows: list[dict[str, object]], disabled: list[str] | None = None) -> list[dict[str, object]]:
    ...
    for column in display_frame.columns:
        if column in whole_number_fields:
            display_frame[column] = display_frame[column].map(lambda value: float(round(safe_float(value))))
        elif column in one_decimal_fields | percent_fields:
    ...
    for index, edited_row in enumerate(edited_rows):
        if index >= len(original_rows):
            continue
        for column in whole_number_fields | one_decimal_fields | percent_fields:
            if column not in edited_row or column not in displayed_rows[index]:
                continue
...
def render_investment_settings(case_id: str, inputs: dict[str, object]) -> dict[str, object]:
    ...
    seeds = {
        "case_name": settings.get("Case Name", ""), "archetype": settings.get("Case Archetype", CASE_ARCHETYPES[0]),
        "drivers": settings.get("Value Creation Drivers", []), "scope": settings.get("Financial Scope", ""),
        "currency": settings.get("Currency", "USD"), "base_year": int(settings.get("Base Year", 2026)),
        "horizon": int(settings.get("Forecast Horizon", 10)), "investment_start": settings.get("Investment Start Date", date.today()),
        "operational_start": settings.get("Operational Start Date", date.today()), "tax_rate": safe_float(settings.get("Applicable Tax Rate", .24))*100,
        "inflation": safe_float(settings.get("Planning Inflation", .025))*100,
    }
    for name, value in seeds.items(): investment_seed_widget(f"investment_{case_id}_{name}", value)
    top = st.columns([2.0, 1.0])
    ...
    st.radio(
        "Case Archetype",
        CASE_ARCHETYPES,
        format_func=lambda value: archetype_labels[value],
        horizontal=True,
        key=f"investment_{case_id}_archetype",
        on_change=handle_investment_archetype_change,
        args=(case_id,),
        label_visibility="collapsed",
    )
    ...
    st.caption(f"Model v{settings.get('Model Version','1.0')} · As of {settings.get('As Of Date',date.today())}")
    settings.update({"Case Name":st.session_state[f"investment_{case_id}_case_name"],"Case Archetype":st.session_state[f"investment_{case_id}_archetype"],"Value Creation Drivers":list(st.session_state[f"investment_{case_id}_drivers"]),"Financial Scope":st.session_state[f"investment_{case_id}_scope"],"Currency":st.session_state[f"investment_{case_id}_currency"],"Base Year":int(st.session_state[f"investment_{case_id}_base_year"]),"Forecast Horizon":int(st.session_state[f"investment_{case_id}_horizon"]),"Investment Start Date":st.session_state[f"investment_{case_id}_investment_start"],"Operational Start Date":st.session_state[f"investment_{case_id}_operational_start"],"Applicable Tax Rate":safe_float(st.session_state[f"investment_{case_id}_tax_rate"])/100,"Planning Inflation":safe_float(st.session_state[f"investment_{case_id}_inflation"])/100,"Model Basis":"Nominal"})
...
def render_investment_model(case: dict[str, object]) -> None:
    ...
    st.caption("Defines the scope, timing and core planning assumptions used across the investment model.")
    ...
    st.caption("Captures the upfront and ongoing capital required to deliver the investment and feeds project cash flow and return calculations.")
    ...
    st.metric("Total Initial Investment", money(total_initial))
    if total_initial <= 0:
    ...
    archetype = str(inputs["settings"].get("Case Archetype"))
    enabled = set(inputs["settings"].get("Value Creation Drivers",[]))
    archetype_context = {
        CASE_ARCHETYPES[0]: (
            "Capacity Expansion Economics",
            "Model incremental capacity, utilization, volume and realized net revenue.",
        ),
        CASE_ARCHETYPES[1]: (
            "Digital / AI Value Creation",
            "Model implementation cost, recurring technology cost and realized productivity / cost savings.",
        ),
        CASE_ARCHETYPES[2]: (
            "Acquisition Economics",
            "Model target standalone performance, integration costs and revenue / cost synergies.",
        ),
    }
    context_title, context_description = archetype_context.get(archetype, archetype_context[CASE_ARCHETYPES[0]])
    ...
    if driver_names:
        ...
        render_finance_table(format_investment_driver_table(calculated,years),right_align=set(years))
    ...
    st.caption("Current rates shown from lowest to highest.")
...
def _financing_ratio(value: object) -> str:
    if value is None or pd.isna(value):
        return "N/A"
    return f"{safe_float(value):.2f}x"


def _financing_percent(value: object) -> str:
    if value is None or pd.isna(value):
        return "N/A"
    return pct(value)


def _format_financing_money_table(table: pd.DataFrame, text_columns: set[str]) -> pd.DataFrame:
    formatted = table.astype(object).copy()
...
def _investment_financing_widget(key: str, value: object) -> str:
    investment_seed_widget(key, value)
    return key


def render_investment_financing(case: dict[str, object]) -> None:
    case_id = str(case.get("Case ID", ""))
    model_inputs = investment_case_inputs(case)
    financing = ensure_financing_inputs(model_inputs.get("financing"))
    scenarios = financing["scenarios"]
    ...
    with st.expander("Financing definitions", expanded=False):
        st.markdown(
            ...
            "**Debt Facility:** maximum committed lender funding; **Debt Drawdown:** amount actually borrowed in a period; **Outstanding Debt:** principal remaining after repayments.  \n"
    ...
    selected_key = f"investment_financing_selected_{case_id}"
    _investment_financing_widget(selected_key, financing.get("selected_scenario_id", "mixed"))
    if st.session_state[selected_key] not in scenario_ids:
        st.session_state[selected_key] = "mixed" if "mixed" in scenario_ids else scenario_ids[0]
    selected_id = str(st.session_state[selected_key])
    financing["selected_scenario_id"] = selected_id
    kpi_slot = st.container()

    st.markdown("<div class='enterprise-section-title'>1. Funding Structure</div>", unsafe_allow_html=True)
    st.caption("Set the debt share of the upfront funding requirement. Internal Cash / Equity is the residual after Debt and optional Other Funding.")
    ...
    debt_control = st.columns([1.1, 2.9])[0]
    debt_control.number_input("Debt %", min_value=0.0, max_value=100.0, step=5.0, key=debt_mix_key, help="Share of Total Initial Funding Need funded by the debt facility. Use the adjacent +/- controls to adjust the mix.")
    ...
    funding_mix_slot = st.container()

    debt_pct = safe_float(selected_mix.get("Debt %"))
    st.markdown("<div class='enterprise-section-title'>2. Debt Terms</div>", unsafe_allow_html=True)
    ...
    if debt_pct <= 0:
        ...
        rate_type_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_rate_type", debt_terms.get("Interest Rate Type", "Fixed"))
        term_columns[0].selectbox("Interest Rate Type", ["Fixed", "Floating"], key=rate_type_key)
        debt_terms["Interest Rate Type"] = st.session_state[rate_type_key]
        if debt_terms["Interest Rate Type"] == "Fixed":
            fixed_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_fixed_rate", safe_float(debt_terms.get("Fixed Interest Rate")) * 100)
            term_columns[1].number_input("Fixed Interest Rate %", min_value=0.0, step=0.1, key=fixed_key, help="Contractual interest rate fixed for the modeled facility term.")
            debt_terms["Fixed Interest Rate"] = safe_float(st.session_state[fixed_key]) / 100
        else:
            reference_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_reference_rate", safe_float(debt_terms.get("Reference / Base Rate")) * 100)
            spread_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_credit_spread", safe_float(debt_terms.get("Credit Spread")) * 100)
            term_columns[1].number_input("Reference / Base Rate %", min_value=0.0, step=0.1, key=reference_key, help="Benchmark floating interest rate before the borrower-specific credit spread.")
            term_columns[2].number_input("Credit Spread %", min_value=0.0, step=0.1, key=spread_key, help="Additional interest margin for borrower or transaction credit risk.")
            debt_terms["Reference / Base Rate"] = safe_float(st.session_state[reference_key]) / 100
            debt_terms["Credit Spread"] = safe_float(st.session_state[spread_key]) / 100

        terms = st.columns(4)
        start_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_facility_start", debt_terms.get("Facility Start / Availability Date", date.today()))
        maturity_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_maturity", int(safe_float(debt_terms.get("Maturity Year")) or 1))
        grace_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_grace", int(safe_float(debt_terms.get("Grace Period Years"))))
        repayment_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_repayment", debt_terms.get("Repayment Type", "Straight-line"))
        terms[0].date_input("Facility Start / Availability Date", key=start_key)
        terms[1].number_input("Maturity (year from drawdown)", min_value=1, max_value=10, step=1, key=maturity_key, help="Final modeled year in which outstanding principal must be repaid.")
        terms[2].number_input("Grace Period (years)", min_value=0, max_value=9, step=1, key=grace_key, help="Period during which principal repayment is deferred.")
        terms[3].selectbox("Repayment Type", ["Straight-line", "Bullet", "Custom"], key=repayment_key, help="Straight-line repays broadly equal principal amounts; Bullet repays principal substantially at maturity.")
        debt_terms.update({
            "Facility Start / Availability Date": st.session_state[start_key],
            "Maturity Year": int(st.session_state[maturity_key]),
            "Grace Period Years": int(st.session_state[grace_key]),
            "Repayment Type": st.session_state[repayment_key],
        })
        fees = st.columns(4)
        arrangement_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_arrangement", safe_float(debt_terms.get("Arrangement Fee")) * 100)
        ...
        fees[3].checkbox("Prepayment Allowed", key=prepay_key, help="Contractual ability to repay principal earlier than scheduled. It does not trigger automatic repayment from excess project cash; principal follows the selected repayment schedule unless an explicit sweep is modeled.")
        debt_terms.update({
        ...
        st.selectbox(
            ...
            key=tax_shield_key,
            help="Immediate assumes the wider group can monetize the interest deduction against broader taxable income. Project-only caps the shield at tax actually avoided by the modeled project.",
        )
        ...
        if debt_terms["Interest Rate Type"] == "Floating":
            st.caption(f"Floating All-in Interest Rate = Reference / Base Rate {pct(debt_terms.get('Reference / Base Rate'))} + Credit Spread {pct(debt_terms.get('Credit Spread'))} = {pct(all_in_rate)}.")
        else:
            st.caption(f"All-in Interest Rate: {pct(all_in_rate)}")
        st.caption("Prepayment permission does not accelerate debt automatically. Contractual principal follows Repayment Type, Grace Period and Maturity unless a separate cash-sweep rule is explicitly introduced.")
    selected["Debt Terms"] = debt_terms

    st.markdown("<div class='enterprise-section-title'>3. Sources & Uses</div>", unsafe_allow_html=True)
    st.caption("Reconciles the upfront funding requirement to Model → Investment Uses. Financing adds the debt arrangement fee and calculates residual Internal Cash / Equity automatically.")
    ...
    covenant_editor = st.data_editor(
        ...
        hide_index=True,
        use_container_width=True,
        disabled=["Covenant", "Direction"],
        column_config={
            "Calculation Basis": st.column_config.SelectboxColumn("Calculation Basis", options=["Project", "Relevant Business Scope", "Borrower / Company"], required=True),
            "Limit": st.column_config.NumberColumn("Covenant Limit", min_value=0.0, format="%.2f"),
        },
    )
    selected["Covenants"] = covenant_editor.to_dict("records")
    ...
    override["Enabled"] = bool(st.session_state[override_key])
    if override["Enabled"]:
        override_columns = st.columns([1, 2])
        rate_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_equity_rate", safe_float(override.get("Rate")) * 100)
        rationale_key = _investment_financing_widget(f"investment_financing_{case_id}_{selected_id}_equity_rationale", str(override.get("Rationale", "")))
        override_columns[0].number_input("Equity Discount Rate %", min_value=0.0, step=0.1, key=rate_key)
        override_columns[1].text_input("Override Rationale", key=rationale_key)
        override["Rate"] = safe_float(st.session_state[rate_key]) / 100
        override["Rationale"] = str(st.session_state[rationale_key])
        if not override["Rationale"].strip():
            st.warning("A short rationale is required for an equity discount-rate override.")
    selected["Equity Discount Rate Override"] = override
    ...
    save_investment_case_inputs(case_id, model_inputs)
    operating_model = calculate_investment_model(model_inputs)
    selected_result = calculate_financing_scenario(operating_model, financing, selected_id)
    all_results = calculate_all_financing_scenarios(operating_model, financing)
    ...
    with kpi_slot:
        ...
        references = st.columns(4)
        references[0].metric("Project NPV · Reference", money(metrics["Project NPV"]))
        references[1].metric("Project IRR · Reference", _financing_percent(metrics["Project IRR"]))
        references[2].metric("Corporate Hurdle Rate", pct(model_inputs["capital"].get("Corporate Hurdle Rate")))
        references[3].metric("Marginal Reinvestment Return", pct(model_inputs["capital"].get("Marginal Reinvestment Return")))

    with funding_mix_slot:
    ...
    with sources_uses_slot:
        ...
        with columns[0]:
            st.markdown("**Uses**")
            uses = selected_result["uses"].copy()
            uses.loc[len(uses)] = {"Use": "Total Uses", "Amount": metrics["Total Uses"], "Type": "Calculated"}
            render_finance_table(_format_financing_money_table(uses, {"Use", "Type"}), right_align={"Amount"})
        with columns[1]:
            st.markdown("**Sources**")
    ...
    with debt_schedule_slot:
        if debt_pct <= 0:
            st.caption("Debt schedule: Not applicable for this scenario.")
        else:
            debt_summary = st.columns(5)
            debt_summary[0].metric("Debt Facility", money(metrics["Debt Funding"]), help="Maximum committed debt available from the lender under this modeled scenario.")
            ...
            debt_summary[3].metric("Total Contractual Debt Service", money(metrics["Total Contractual Debt Service"]), help="Cumulative scheduled principal repayments plus cumulative cash interest. Upfront arrangement fees are excluded.")
            debt_summary[4].metric("Debt Fully Repaid", str(metrics["Debt Fully Repaid Year"]))
            ...
            st.caption("Average Debt = (Opening Debt + Closing Debt) / 2. Cash Interest = Average Debt × All-in Interest Rate. This is an annual approximation for periods with intra-year drawdowns or repayments.")
            st.caption("Commitment Fee applies only to undrawn committed debt. A facility fully drawn at Y0 has no commitment fee after drawdown. Upfront arrangement fees are shown separately and are not included in Total Contractual Debt Service.")
    ...
    with levered_slot:
        ...
        st.caption("Principal repayment is a financing cash flow, not an expense; it appears in Equity Cash Flow, not Levered P&L.")
    ...
    with equity_slot:
        ...
        st.caption("Equity Cash Flow = Unlevered FCF + Debt Drawdown + Alternative Funding Inflow - Principal Repayment - Alternative Funding Repayment - Cash Interest + Interest Tax Shield - Financing Fees - Other Initial Uses.")
        st.caption("Initial Equity Contribution is the residual Y0 funding need. Additional Equity Support funds later negative shareholder cash flows. Total Equity Contributions = Initial Equity Contribution + Additional Equity Support.")
        st.caption("Equity IRR is the IRR of the shareholder Equity Cash Flow series. Equity NPV discounts that series at Cost of Equity. Equity Payback is the first point when cumulative undiscounted Equity Cash Flow becomes positive.")
        st.caption("The interest tax shield is shown once in Equity Cash Flow and is not included in standalone project returns. Immediate / Group availability monetizes the interest deduction against broader taxable income; Project-only availability is capped at modeled project cash tax.")

    with comparison_slot:
        comparison_rows = []
        for scenario_id in scenario_ids:
            result_metrics = all_results[scenario_id]["metrics"]
            comparison_rows.append({
                "Funding Scenario": "Current Funding Structure" if scenario_id == selected_id else scenarios[scenario_id].get("Scenario Name", scenario_id),
                "Debt %": pct(all_results[scenario_id]["scenario"]["Funding Mix"].get("Debt %")),
                ...
                "Cumulative Interest": money(result_metrics["Interest Cost"]),
                ...
                "Covenant Breach": "Yes" if result_metrics["Covenant Breaches"] else "No",
    ...
    with interpretation_slot:
        scenario_name = "the current funding structure"
...
def page_new_investment_case() -> None:
    ...
    fields = st.columns(3)
    investment_type = fields[0].selectbox("Case Archetype", CASE_ARCHETYPES, key="new_investment_type")
    ...
    if st.button("Create Investment Case", key="create_investment_case", disabled=not name.strip() or not business_unit.strip()):
        ...
        new_case = {"Case ID": case_id, "Investment Case Name": name.strip(), "Case Archetype": investment_type, "Investment Type": investment_type.replace(" / New Production Line", "").replace(" Investment", ""), "Business Unit / Market": business_unit.strip(), "Owner": owner, "Status": "Draft", "Last Updated": date.today().isoformat(), "Profile": "digital" if investment_type == CASE_ARCHETYPES[1] else "capacity"}
